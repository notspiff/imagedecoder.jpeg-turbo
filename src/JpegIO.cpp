...
bool CJpegIO::Open(const std::string &texturePath, unsigned int minx, unsigned int miny, bool read)
{
  ...
  void* file = XBMC->OpenFile(texturePath.c_str(), 0);
  m_inputBuffSize = XBMC->GetFileLength(file);
  m_inputBuff = new unsigned char[m_inputBuffSize];
  XBMC->ReadFile(file, m_inputBuff, m_inputBuffSize);
  XBMC->CloseFile(file);
...
bool CJpegIO::Read(unsigned char* buffer, unsigned int bufSize, unsigned int minx, unsigned int miny)
{
  ...
  */
    ...
    unsigned int maxtexsize = 2048;//g_Windowing.GetMaxTextureSize();
...
bool CJpegIO::CreateThumbnailFromMemory(unsigned char* buffer, unsigned int bufSize, const std::string& destFile, unsigned int minx, unsigned int miny)
{
  ...
  if (!Read(buffer, bufSize, minx, miny))
    return false;
  pitch = m_width * 3;
  sourceBuf = new unsigned char [m_height * pitch];

  if (!Decode(sourceBuf, m_width, m_height,pitch,XB_FMT_RGB8))
  {
    delete [] sourceBuf;
    return false;
  }
  if (!CreateThumbnailFromSurface(sourceBuf, m_width, m_height, XB_FMT_RGB8, pitch, destFile))
  {
    delete [] sourceBuf;
    return false;
  }
...
bool CJpegIO::CreateThumbnailFromSurface(unsigned char* buffer, unsigned int width, unsigned int height, unsigned int format, unsigned int pitch, const std::string& destFile)
{
  ...
  void* file = XBMC->OpenFileForWrite(destFile.c_str(), true);
  XBMC->WriteFile(file, result, outBufSize);
  const bool ret = XBMC->WriteFile(file, result, outBufSize) == static_cast<ssize_t>(outBufSize);
  XBMC->CloseFile(file);
  free(result);
...
{
  char msg[1024];
  sprintf(msg, "Error %i: %s",cinfo->err->msg_code, cinfo->err->jpeg_message_table[cinfo->err->msg_code]);
            ...
            unsigned int height, unsigned int pitch, unsigned int format)
{
  if (!image)
    return false;
  CJpegIO* jpeg = static_cast<CJpegIO*>(image);

  return jpeg->Decode(pixels, width, height, pitch, format);
}
                     ...
                     const char* destFile)
{
  ...
  return jpeg.CreateThumbnailFromSurface(bufferin, width, height, format, pitch, destFile, bufferout, bufferoutSize); 
}

void Close(void* image)
{
  delete static_cast<CJpegIO*>(image);
}
